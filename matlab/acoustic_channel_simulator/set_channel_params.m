...
%
% Contains slight modifications for av-uwa simulations by Adam Gannon, 2018.
...
h0=10; % surface height (depth) [m]
ht0=4; % TX height [m]
hr0=2; % RX height [m]
d0=600; % channel distance [m]

k=1.7; % spreading factor
c=1500; % speed of sound in water [m/s]
c2=1500; % speed of sound in bottom [m/s] (>1500 for hard, < 1500 for soft)
cut=20; % do not consider arrivals whose strength is below that of direct arrival divided by cut

fmin=95e3; % minimum frequency [Hz]
B=50e3;%10e3; % bandwidth [Hz]

% Make the time resolution exact multiples of Tc
alpha = 0.35;
Tc = (1+alpha)/B;
code_common_multiple = 64*3; % Common to code choices [8,12,16,24,32,64]
dt= code_common_multiple*Tc; %0.01 % time resolution [seconds] 
